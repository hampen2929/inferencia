...
class MultiTracker:
    ...
    def apply_kalman(self):
        ...
        for trk_id, track in list(self.tracks.items()):
            ...
            #         mean, cov, flow_tlbr, MeasType.FLOW, std_multiplier)
    ...
    def update(self, frame_id, detections, embeddings):
        ...
        confirmed = [trk_id for trk_id,
                     track in self.tracks.items() if track.confirmed]
        unconfirmed = [trk_id for trk_id,
                       track in self.tracks.items() if not track.confirmed]

        # association with motion and embeddings
        cost = self._matching_cost(confirmed, detections, embeddings)
        matches1, u_trk_ids1, u_det_ids = self._linear_assignment(
            cost, confirmed, det_ids)

        # 2nd association with IoU
        active = [trk_id for trk_id in u_trk_ids1 if self.tracks[trk_id].active]
        u_trk_ids1 = [
            trk_id for trk_id in u_trk_ids1 if not self.tracks[trk_id].active]
        u_detections = detections[u_det_ids]
        cost = self._iou_cost(active, u_detections)
        matches2, u_trk_ids2, u_det_ids = self._linear_assignment(
            cost, active, u_det_ids, True)
        ...
        u_det_ids = [
            det_id for det_id in u_det_ids if detections[det_id].conf >= self.conf_thresh]
        u_detections, u_embeddings = detections[u_det_ids], embeddings[u_det_ids]
        cost = self._reid_cost(u_detections, u_embeddings)
        reid_matches, _, u_det_ids = self._linear_assignment(
            cost, lost_ids, u_det_ids)
        ...
        for trk_id, det_id in matches:
            ...
            mean, cov = self.kf.update(
                *track.state, det.tlbr, MeasType.DETECTOR)
            next_tlbr = as_rect(mean[:4])
    ...
    def _matching_cost(self, trk_ids, detections, embeddings):
        ...
        for i, trk_id in enumerate(trk_ids):
            track = self.tracks[trk_id]
            motion_dist = self.kf.motion_distance(
                *track.state, detections.tlbr)
    ...
    def _iou_cost(self, trk_ids, detections):
        ...
        trk_labels = np.array(
            [self.tracks[trk_id].label for trk_id in trk_ids])
        ...
        ious = self._gate_cost(
            ious, trk_labels, detections.label, self.iou_thresh, True)
        return ious
    ...
    def _reid_cost(self, detections, embeddings):
        ...
        cost = cdist(features, embeddings, self.metric)
        cost = self._gate_cost(
            cost, trk_labels, detections.label, self.max_reid_cost)
        return cost
    ...
    def _remove_duplicate(self, updated, aged):
        ...
        updated_bboxes = np.array(
            [self.tracks[trk_id].tlbr for trk_id in updated])
    ...
    def _fuse_motion(cost, motion_dist, det_labels, label, age, max_cost, w1, w2):
        gate = (cost > max_cost) | (motion_dist >
                                    CHI_SQ_INV_95) | (label != det_labels)
